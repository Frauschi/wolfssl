...
* Added RFC 9629 KEMRecipientInfo with ML-KEM to PKCS#7/CMS, so an EnvelopedData or AuthEnvelopedData can encapsulate the content-encryption key to a recipient's ML-KEM certificate. Together with the existing RFC 9882 ML-DSA SignedData support this covers both halves of a CNSA 2.0 S/MIME message. A registered `wc_PKCS7_SetOriDecryptCb` callback is tried first, and the built-in decoder takes over when it fails. by @Frauschi
