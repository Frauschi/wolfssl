...
* Fixed AuthEnvelopedData decoding to step over the RecipientInfo structures left unread after the matching one. Any AuthEnvelopedData with more than one recipient previously failed to parse the content that follows the recipient set, whatever the recipient type. by @Frauschi
* Fixed the stream read accounting after a RecipientInfo is rejected. A handler stops counting as soon as it decides the recipient is not the reader's, but the index has already stepped over the whole structure, so the next handler's own bookkeeping compounded the difference and the decode asked for input it had already consumed. An AuthEnvelopedData whose matching recipient was not the first failed with `WC_PKCS7_WANT_READ_E` in a streaming build. by @Frauschi
* Bounded the recipient search by the RecipientInfo set, so the EncryptedContentInfo that follows it is no longer read as another recipient. A message with no recipient for the reader now reports `PKCS7_RECIP_E` rather than a parse error from the misread content. by @Frauschi
* Accepted the RFC 9935 seed and "both" private-key CHOICE forms when decoding an ML-KEM PKCS#8 key, expanding the 64-byte seed with ML-KEM.KeyGen_internal(d,z). A key carrying both a seed and an expanded key is rejected when the two disagree, per RFC 9935 Section 8. A `WOLFSSL_MLKEM_NO_MAKE_KEY` build cannot expand a seed and so cannot run that comparison; it rejects every key carrying a seed rather than accepting the expanded half unchecked. by @Frauschi
