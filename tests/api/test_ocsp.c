...
static int test_ocsp_staple_crl_policy_norevoke_ctx_ready(WOLFSSL_CTX* ctx)
{
    EXPECT_DECLS;
...
int test_ocsp_staple_crl_policy(void)
{
    ...
    test_ctx.c_cb.ctx_ready = test_ocsp_staple_crl_policy_norevoke_ctx_ready;
    test_ctx.c_cb.ssl_ready = test_ocsp_staple_crl_policy_ssl_ready;
    ExpectIntEQ(test_ssl_memio_setup(&test_ctx), TEST_SUCCESS);
    ExpectIntNE(test_ssl_memio_do_handshake(&test_ctx, 10, NULL),
        TEST_SUCCESS);
...
int test_ocsp_unstapled_chain_crl(void)
{
    ...
    struct {
        ...
        { wolfTLSv1_3_client_method, wolfTLSv1_3_server_method,
          ...
          NULL, WC_NO_ERR_TRACE(CRL_MISSING) },
        { wolfTLSv1_3_client_method, wolfTLSv1_3_server_method,
...
int test_ocsp_checkall_staple_missing_chain(void)
{
    ...
    for (i = 0; i < XELEM_CNT(params) && !EXPECT_FAIL(); i++) {
        for (unknown = 0; unknown <= 1 && !EXPECT_FAIL(); unknown++) {
            ...
            if (EXPECT_FAIL()) {
                fprintf(stderr, "case: %s, unknown %d\n", params[i].desc,
                    unknown);
            }
