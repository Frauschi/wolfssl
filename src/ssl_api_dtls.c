                       ...
                       const byte* suite)
{
    ...
    /* MakeTlsMasterSecret() does not clean the pre-master secret and the
     * buffer now lives as long as the object, so wipe the caller's secret
     * here. This is the multicast entry point: what was copied in is a
     * long-lived group secret, not a per-connection ephemeral. */
