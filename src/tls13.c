                                       ...
                                       word32 reqCtxLen)
{
    ...
    /* The extension writes WOLFSSL_SUITES(ssl) so wolfSSL_set1_sigalgs_list()
     * is honored. */
...
static int SendTls13Finished(WOLFSSL* ssl)
{
    ...
    if (ssl->options.side == WOLFSSL_SERVER_END) {
         ...
         * DeriveHandshakeSecret() wrote the handshake secret over this buffer
         * without updating preMasterSz, so wipe all of it. */
        ...
        if (ssl->options.side == WOLFSSL_CLIENT_END) {
            ...
            if (type == finished) {
                if ((ret = DeriveMasterSecret(ssl)) != 0)
                    return ret;
                /* Last use of preMasterSecret - zeroize as soon as possible.
                 * DeriveHandshakeSecret() wrote the handshake secret over this
                 * buffer without updating preMasterSz, so wipe all of it. */
                ForceZero(ssl->arrays->preMasterSecret, MAX_PREMASTER_SZ);
        #ifdef WOLFSSL_EARLY_DATA
        #ifdef WOLFSSL_QUIC
