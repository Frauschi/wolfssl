...
/* The C macro alone, which is what the port itself keys off. The port source
 * is compiled unconditionally, so the macro is the single thing that decides
 * whether there is a port to register - and a user settings file that defines
 * it gets this hook without needing the Kconfig option too. */
