...
int main(void)
{
    ...
    /* ---- HMAC-SHA256 ---------------------------------------------------
     * The port has no HMAC arm: ELS HMAC is one-shot while wolfCrypt drives it
     * incrementally. Declining costs nothing, because hmac.c passes its devId
     * down to the inner SHA-256, which is accelerated. Assert that. */
    {
     ...
     * The callback sits in wc_RsaFunction_ex, below all padding. A private op
     * followed by the public op must return the original block. */
    {
        static RsaKey rsa;
        static byte   sig[256];
        static byte   rt[256];
     ...
     * A curve that builds is not a curve that works: with ECC_USER_CURVES and
     * a minimum-key-size filter, a filtered curve vanishes from wolfCrypt's
     * table. Sign and verify on each. */
    {
        ...
        if (wc_InitRng(&rng) == 0) {
            for (c = 0; curves[c].name != NULL; c++) {
                 ...
                 * positive test, so require a rejection too, while the key is
                 * still live. */
                if (r == 0 && verified) {
                    int bad = 1;
                    sig[sigSz - 1] ^= 0x01;
     ...
     * Only keys the device makes for itself. No blob from outside is ever
     * unwrapped, which matters because a malformed container does not fail -
     * it resets the SoC. */
    {
        ...
        /* keySz selects the size the derivation produces; the 0 above took
         * the 128-bit default. A 256-bit key spans the slot pair, so this one
         * lands in 12 and 13, which nothing else here writes. */
        {
            wc_ElsPkc_KeyRef k256;
            byte   ref256[WC_ELSPKC_KEYREF_SZ];
            word32 sz256 = sizeof(ref256);
            word32 kt2 = 0, ks2 = 0, at2 = 0;

            memset(&k256, 0, sizeof(k256));
            k256.keyClass = WC_ELSPKC_KEY_AES;
            k256.slot     = 12;
            (void)wc_ElsPkc_MakeKeyRef(&k256, ref256, &sz256);
            (void)wc_KeyStore_Delete(WOLFSSL_ELS_PKC_DEVID, ref256,
                                     WC_ELSPKC_KEYREF_SZ, NULL);

            r = wc_KeyStore_Derive(WOLFSSL_ELS_PKC_DEVID,
                                   ref256, WC_ELSPKC_KEYREF_SZ,
                                   WC_KEYSTORE_KEY_AES, 32,
                                   dukRef, WC_ELSPKC_KEYREF_SZ,
                                   0, deriv2, sizeof(deriv2), 0, NULL);
            check("256-bit key derived", r == 0);

            r = wc_KeyStore_GetInfo(WOLFSSL_ELS_PKC_DEVID, ref256,
                                    WC_ELSPKC_KEYREF_SZ, &kt2, &ks2, &at2,
                                    NULL);
            check("GetInfo reports 256 bits", r == 0 && ks2 == 256);

            /* A size the engine has no encoding for is declined rather than
             * rounded to one it does. */
            r = wc_KeyStore_Derive(WOLFSSL_ELS_PKC_DEVID,
                                   ref256, WC_ELSPKC_KEYREF_SZ,
                                   WC_KEYSTORE_KEY_AES, 24,
                                   dukRef, WC_ELSPKC_KEYREF_SZ,
                                   0, deriv2, sizeof(deriv2), 0, NULL);
            check("derive REFUSES a size ELS cannot encode", r != 0);
