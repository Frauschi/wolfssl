 ...
 * naming it. Delete it with wc_KeyStore_Delete() when done. A blob wrapped by
 * NXP's provisioning tooling has not been round-tripped against this. */
 ...
 * its result agrees with software. Incremented without atomics and some
 * outside the lock, so best-effort under concurrency. */
...
/* Bring the EdgeLock subsystem up and register the crypto callback. Must run
 * after wolfCrypt_Init(), which zeroes the device table. Safe to call more
 * than once. */
