...
/* Map an ML-KEM algorithm OID sum to its parameter set.
 * Return 0 on success, BAD_FUNC_ARG when the OID names something else */
static int wc_PKCS7_MlKemType(int oidSum, int* type)
{
    int ret = 0;
                                        ...
                                        void* heap, int devId)
{
    ...
    ret = wc_PKCS7_MlKemType((int)decoded->keyOID, &level);
    if (ret != 0) {
                                ...
                                const byte* ukm, word32 ukmSz, int options)
{
    ...
    if ((wrapOID != AES128_WRAP && wrapOID != AES192_WRAP &&
            wrapOID != AES256_WRAP && !wc_PKCS7_WrapIsPadded(wrapOID)) ||
            kekLen <= 0 || (word32)kekLen > sizeof(kek))
        return BAD_KEYWRAP_ALG_E;
                                 ...
                                 byte* decryptedKey, word32* decryptedKeySz)
{
    ...
    if (GetAlgoId(in, &idx, &kemOID, oidKeyType, inSz) < 0)
        return ASN_PARSE_E;
    ret = wc_PKCS7_MlKemType((int)kemOID, &level);
    if (ret != 0) {
         ...
         * "not mine" so the caller keeps looking. wc_PKCS7_MlKemType
        ...
        case WC_PKCS7_DECRYPT_ORI:
            ...
            *idx += oriValueSz;
            ...
            if (ret != 0 || decryptedKey == NULL || *decryptedKeySz == 0) {
                /* ML-KEM decapsulation always succeeds, so a message for
                 * someone else only shows up as a failed key unwrap */
                *recipFound = 0;
                return PKCS7_RECIP_E;
            }
