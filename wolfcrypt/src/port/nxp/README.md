...
**Two derivation rules the hardware enforces.** The ELS mixes the requested property word into every CKDF, so the same parent and derivation data give a different key when the protection bits or `wrpok` differ (`kactv` makes no difference). And only the die unique key in slot 0 may parent a wrap-capable KWK: deriving one from any other key trips the tamper controller, so `wc_KeyStore_Derive()` refuses it with `BAD_STATE_E`. An unwrap-only KWK (`WC_KEYSTORE_ATTR_UNWRAP_ONLY`) derives from any `WC_ELSPKC_KEY_CKDF` source.

**EdgeLock 2GO.** Keys provisioned through NXP's EdgeLock 2GO service descend from `NXP_DIE_EL2GOOEM_MK_SK`, which the boot ROM leaves in slots 4 and 5 only when secure boot is enabled (RW61x user manual UM11865, section 12.11, table 99). The port carries no EdgeLock 2GO code. An application derives the import keys from that slot with `wc_KeyStore_Derive()`, checks each secure object's CMAC with a slot bound through `wc_ElsPkc_CmacUseSlot()`, and unwraps the key container with `wc_KeyStore_ImportWrapped()`; the port's derive properties are the ones NXP's chain uses, so the keys come out identical. A P-256 key arrives as a `WC_ELSPKC_KEY_ECC_SEED` holding the private scalar, and `wc_ElsPkc_EccKeyGenFromSeed()` turns it into a key pair in the same slots and returns the public point; the ELS marks the result wrap-exportable by itself. The derivation data and the secure object format are in NXP's EdgeLock 2GO agent, https://github.com/NXP/el2go-agent.
