...
out:
    if (ret != 0) {
        /* Bytes are counted in total that the engine never saw, so anything
         * this context produces from here is wrong. Mark it rather than let
         * a caller that ignores this return get a confident bad digest. */
        ctx->failed = 1;
    }
    ElsUnlock();
                        ...
                        int mode, int encrypt, int useIv)
{
    ...
    if (ret == 0) {
        /* Carry the CBC chaining state forward ourselves.
         *
         * The documentation says ELS "will always read and write" pIV for CBC.
         * It reads it, but on this part it does not write it back: after a
         * one-block encrypt, aes->reg still held the original IV while the
         * software path had advanced to the ciphertext block. A chained call
         * would therefore restart from the original IV and silently produce
         * wrong output - correct for the first call, wrong from the second.
         *
         * Next IV is the last ciphertext block either way: the output when
         * encrypting, the (saved) input when decrypting. */
...
static int ElsEccKeyGen(ecc_key* key)
{
    ...
    /* Ask the hardware about the target before writing into it. The rest of
     * this port validates a reference before every command because a bad one
     * resets the part rather than returning an error, and a generation is no
     * different - it just happens to name a slot that should still be empty.
     * An occupied slot is refused: overwriting a key nobody asked to replace
 ...
 * at 536 bytes (4096-bit, NoEncode) and ECC at 504. Sized once rather than per
 * call, because it is shared state under the lock either way. */
                           ...
                           byte* out)
{
     ...
     * the padded modes, which actually check a signature. Accepting only
     * VERIFY_OK rejects every successful raw public operation. */
...
/* RSA at the raw primitive.
 *
 * wolfCrypt's callback sits in wc_RsaFunction_ex - beneath all padding - so the
 * matching vendor modes are the ones that also skip it: NoVerify is RSAVP1 and
 * NoEncode is RSASP1. That happens to be the well-behaved corner of this API:
 * every encrypt and decrypt workarea macro on rw61x still carries an
 * unsubstituted template placeholder and will not compile at all.
 */
                             ...
                             word32* outLen, int type, RsaKey* key)
{
    ...
    if (!isPrivate) {
        ...
        /* private: CRT if the parameters are there, plain otherwise. CRT is
         * the whole point of offloading a private operation, so only fall back
         * when the key genuinely lacks the factors. */
                        ...
                        byte* out, word32* outLen, int endian)
{
    ...
    /* The engine works little-endian, which is X25519's own wire order.
     * A big-endian request is wolfCrypt's legacy convention; decline it
     * rather than byte-swapping a shared secret behind the caller's back. */
                         ...
                         word32* outlen, ecc_key* key)
{
    ...
    /* Two separate bounds, and only one of them is the engine's. byteLenHash
     * occupies eight bits of optLen, but the digest is also copied into a
     * fixed buffer here - so the buffer is the tighter limit, and checking
     * only the field width would overflow the stack for anything larger. */
 ...
 * Key store
 *
 * The operations that cross the vault boundary, or deliberately stay inside
 * it. None of them can be expressed by an algorithm callback: import and
 * export carry a wrapped container that never becomes plaintext on this side,
 * and derive moves a key from one slot to another without it entering memory
 * at all.
 *
 * Every entry point validates its references against the hardware first. That
 * matters more here than anywhere else in the port: these calls name two slots
 * rather than one, and ELS answers a permission mistake by resetting the SoC.
 * ------------------------------------------------------------------------ */
...
/* Find room for a key that does not exist yet.
 *
 * PSA's generate has no way for a caller to say which slot to use, so the
 * driver has to choose - the same conclusion NXP reached for their own PSA
 * driver, where an integrator-supplied Oracle allocates the slot.
 *
 * The store is not empty on a fresh part: the device-unique key, a
 * hardware-output slot and others are occupied out of the box, so the only
 * safe test for "free" is the hardware's own active bit. A 256-bit key spans
 * two consecutive slots, so an asymmetric class needs a consecutive pair.
 *
 * The result is advisory: nothing marks a slot as taken until a key is
 * actually written into it, so two reservations made before either is used
 * can name the same slot. Reserve and generate together.
 */
int wc_ElsPkc_ReserveSlot(byte keyClass, wc_ElsPkc_KeyRef* ref)
{
    ...
    for (slot = 0; slot <= WC_ELSPKC_MAX_SLOT; slot++) {
         ...
         * slot 2 is hardware-out on this part, holding the die key and the
         * flash encryption key; an ordinary key does not belong in any of
         * them. Only general purpose slots are candidates. */
        if ((prop.word.value &
...
/* The wrapping key is its own class, and the two ELS permissions behind it are
 * not interchangeable: ukwk both wraps and unwraps, ukuok only unwraps. An
 * export therefore needs more than an import does, which is the whole point of
 * an unwrap-only key. */
...
int wc_ElsPkc_DeriveDieKek(wc_ElsPkc_KeyRef* ref)
{
    ...
    if (ret == 0) {
        /* Reproduce the ROM's own recipe: a wrapping key, 256 bits, no
         * privilege or secure-mode restriction. Nothing else is requested,
         * because the engine refuses a derivation that asks for properties
         * the parent cannot confer. */
        prop.word.value = 0u;
        prop.bits.upprot_priv = MCUXCLELS_KEYPROPERTY_PRIVILEGED_FALSE;
        prop.bits.upprot_sec  = MCUXCLELS_KEYPROPERTY_SECURE_FALSE;
...
static int ElsKsDelete(wc_CryptoInfo* info)
{
    ...
    if (ret == 0) {
        ...
        ret = ElsKsProps(key.slot, &prop);
        if (ret == 0) {
            /* Deleting an empty slot is a caller mistake, and this is not a
             * device that returns errors for those - so check first and
             * report it here rather than find out from a reset. Nothing is
             * destroyed either way, so it is not treated as a failure. */
            if ((prop.word.value & MCUXCLELS_KEYPROPERTY_VALUE_ACTIVE) == 0) {
                ElsUnlock();
                return 0;
            }
...
{
    /* The DRBG takes at least four bytes and only whole words - the driver
     * rejects anything else, and with its parameter checks compiled out it
     * would program the DMA with a sub-word length and write past the end of
     * the caller's buffer.
     *
     * Declining odd sizes would be safe but wasteful: a single byte is an
     * ordinary request (wc_RNG_GenerateByte), and refusing it here does not
     * fall back to software, because only CRYPTOCB_UNAVAILABLE does that -
     * WC_HW_E would simply fail the call. So the whole-word part is generated
     * straight into the caller's buffer and the few remaining bytes come from
     * a word-sized scratch. */
 ...
 * AES-CMAC
 *
 * Same shape as the hash: pMac is [in, out] and carries the intermediate
 * state, so it lives in the caller's Cmac and the lock is per call. Unlike the
 * hash, ELS does the CMAC padding itself - inputLength is the length "before
 * padding" - so the port only has to withhold the final chunk until it knows
 * it is final, which is what the finalize bit selects the right subkey for.
 * ------------------------------------------------------------------------ */
...
/* Release the pool entry behind an abandoned CMAC.
 *
 * wc_CmacFree() calls the FREE callback for exactly this: a Cmac that was
 * started on hardware and then dropped without a Final - an update that
 * errored, or an application that just frees the struct. Without this the
 * entry stays claimed forever, and since the pool defaults to two entries,
 * two such events disable CMAC offload for the life of the process. The
 * entry also holds a plaintext AES key, which would otherwise sit in .bss
 * with nothing left able to reclaim or zero it. */
...
static int ElsCmacUpdate(Cmac* cmac, const byte* in, word32 inSz)
{
    ...
    /* Never hand the hardware the last block during an update: CMAC uses a
     * different subkey for the final chunk, and only the final call knows
     * which chunk that is. So a full buffer is only flushed once more data is
     * known to follow. */
    while (inSz > 0) {
...
static int ElsCmacFinal(Cmac* cmac, byte* out, word32* outSz)
{
    ...
    /* wc_CmacFinalNoFree() accepts any tag length in [WC_CMAC_TAG_MIN_SZ,
     * WC_CMAC_TAG_MAX_SZ], and the leftmost bytes of the full CMAC are
     * exactly that truncation - which is what wc_AesCmacVerify_ex() asks for.
     * Refusing them here is not a fallback: by this point the message has
     * already been absorbed into the hardware and there is no software state
     * left to fall back to, so the call would simply fail. */
    ...
    /* Apply the SP 800-38B padding ourselves.
     *
     * pInput is documented as "the data, padded via SP 800-38B standard", with
     * inputLength being the length *before* padding - so ELS derives the
     * subkey choice from the length but expects the block to arrive already
     * padded. An incomplete final block therefore needs 0x80 followed by
     * zeros; a complete one is passed untouched.
     *
     * Getting this wrong is not loud: exact block multiples come out correct
     * (they need no padding) and every other length is silently wrong. */
