   ...
   sword32* y_check;
    ...
    if (ret == 0) {
        ...
        do {
             ...
             * The matrix is walked a column at a time so that each polynomial
            ...
            if ((ret == 0) && valid) {
                ...
                for (r = 0; valid && (r < params->k); r++) {
                    ...
                    if (valid) {
                        ...
                        ct0Valid &= mldsa_check_low_ct(ct0, params->gamma2);
                        /* Step 26: ct0 = ct0 + w0 */
                        mldsa_add(ct0, w0t);
                        mldsa_poly_red(ct0);
            ...
            if ((ret == 0) && valid) {
                ...
                for (r = 0; (ret == 0) && valid && (r < params->k); r++) {
                    ...
                    if (valid) {
                        ...
                        /* Step 27: Check ct0 has low enough values. Every
                         * row: ct0 has no mask, so a failing index leaks t0. */
                        ct0Valid &= mldsa_check_low_ct(z, params->gamma2);
                        /* Step 26: ct0 = ct0 + w0 */
                        mldsa_add(z, wt);
                        mldsa_poly_red(z);
