...
/* Pull in the legacy compatibility shim FIRST. dilithium.h runs the
 * sub-config gate translation block (legacy WOLFSSL_DILITHIUM_* /
 * WC_DILITHIUM_* -> canonical WOLFSSL_MLDSA_* / WC_MLDSA_*) and then
 * transitively #include's wc_mldsa.h with the canonical gates correctly
 * set. This must happen before wc_mldsa.h's own conditional declarations
 * are read. The shim also brings the legacy macro / inline aliases into
 * scope for the compile-time wc_mldsa_legacy_alias_check() near the
 * bottom of this file. */
...
/* Compile-time validation of the canonical wc_MlDsaKey_* / MlDsaKey API. The
 * body sits inside `if (0)` so the compiler parses every reference without
 * emitting runtime calls. Each canonical name in <wc_mldsa.h> is exercised
 * exactly once, so a typo or signature drift in the canonical declaration
 * surface produces a build error here rather than silently at a downstream
 * consumer. */
static WC_INLINE void wc_mldsa_canonical_api_check(void)
{
    if (0) {
        MlDsaKey k;
        const MlDsaParams *p;
...
/* Compile-time validation of the dilithium.h legacy alias shim. The
 * shim was already pulled into scope at the top of this file; the
 * function below exercises every legacy macro / typedef / inline so a
 * missing or misordered alias produces an immediate compile error.
 * Bodies sit inside `if (0)` so the compiler parses them without
 * emitting any runtime call; `WC_INLINE` plus the unused-static
 * suppression keeps the function from triggering
 * -Werror=unused-function. */
#if !defined(WOLFSSL_NO_DILITHIUM_LEGACY_NAMES)

static WC_INLINE void wc_mldsa_legacy_alias_check(void)
{
    if (0) {
        MlDsaKey k;
