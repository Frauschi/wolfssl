 ...
 *   Which of the two is quicker depends on the target. Where the C code runs
 *   this is the quicker of the two, as each polynomial of y is transformed
 *   once rather than once per row of matrix A. Where the assembly runs it is
 *   the slower of the two, as generating y a polynomial at a time cannot use
 *   the generators that produce the whole vector in one pass. Prefer this on
 *   targets without a vector unit and WOLFSSL_MLDSA_SIGN_SMALL_MEM elsewhere.
    ...
    word16 kappa, byte gamma1_bits, sword32* y, byte l, void* heap)
{
    ...
    if (IS_INTEL_AVX2(cpuid_flags) && IS_INTEL_BMI2(cpuid_flags) &&
            (SAVE_VECTOR_REGISTERS2() == 0)) {
        /* Each generator handles one dimension only, so anything else, such
         * as the single polynomial the smallest memory signing asks for,
         * falls through to the C implementation. */
        byte done = 0;
        ...
        if (!done) {
            ret = mldsa_vec_expand_mask_c(shake256, seed, kappa, gamma1_bits,
                y, l);
        }
    }
    else
...
static int mldsa_make_key_from_seed(wc_MlDsaKey* key, const byte* seed)
{
    ...
    if (ret == 0) {
        ...
        for (r = 0; (ret == 0) && (r < params->k); r++) {
            ...
            mldsa_vec_decode_eta_bits(s2pt, params->eta, s2t, 1);
            mldsa_add(tt, s2t);
            /* Make positive for decomposing. */
            mldsa_make_pos(tt);
    ...
    const byte* seedMu, byte* sig, word32 *sigLen)
{
   ...
   sword32* y_check;
    ...
    if (ret == 0) {
        ...
        do {
        ...
        #ifdef WOLFSSL_MLDSA_SMALL
            mldsa_vec_red(w, maxK);
        #endif
            mldsa_vec_invntt_full(w, maxK);
            /* Step 14, Step 22: Make values positive and decompose. */
            mldsa_vec_make_pos(w, maxK);
            mldsa_vec_decompose(w, maxK, params->gamma2, w0, w);
            /* Step 15: Encode the w1 polynomials just computed. */
            mldsa_vec_encode_w1(w, maxK, params->gamma2, w1e);
        #endif
            /* Step 5: Create the matrix A from the public seed. */
            /* Copy the seed into a buffer that has space for s and r. */
            XMEMCPY(aseed, pub_seed, MLDSA_PUB_SEED_SZ);
            ...
            if ((ret == 0) && valid) {
                ...
                for (r = 0; valid && (r < params->k); r++) {
                    ...
                    if (valid) {
                    ...
                    #ifndef WOLFSSL_NO_ML_DSA_44
                        if (params->gamma2 == MLDSA_Q_LOW_88) {
                            valid = (mldsa_make_hint_88(ct0, w, h,
                                &idx) == 0);
                        ...
                        !defined(WOLFSSL_NO_ML_DSA_87)
                        if (params->gamma2 == MLDSA_Q_LOW_32) {
                            valid = (mldsa_make_hint_32(ct0, w,
        ...
        do {
            ...
            if ((ret == 0) && valid) {
                ...
                for (r = 0; (ret == 0) && valid && (r < params->k); r++) {
                    ...
                    if (valid) {
                    ...
                    #ifndef WOLFSSL_NO_ML_DSA_44
                        if (params->gamma2 == MLDSA_Q_LOW_88) {
                            valid = (mldsa_make_hint_88(z, a, h, &idx) == 0);
                            /* Alg 14, Step 10: Store count of hints for
                             *                  polynomial at end of list. */
                            h[PARAMS_ML_DSA_44_OMEGA + r] = idx;
                        }
                        ...
                        !defined(WOLFSSL_NO_ML_DSA_87)
                        if (params->gamma2 == MLDSA_Q_LOW_32) {
                            valid = (mldsa_make_hint_32(z, a, params->omega,
                                h, &idx) == 0);
                            /* Alg 14, Step 10: Store count of hints for
                             *                  polynomial at end of list. */
                            h[params->omega + r] = idx;
