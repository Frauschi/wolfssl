    ...
    word32 q, byte* auth_path, byte* pub)
{
    ...
    WC_ALLOC_VAR_EX(stack, byte, LMS_STACK_CACHE_LEN(params->height,
        params->hash_len), NULL, DYNAMIC_TYPE_TMP_BUFFER, ret=MEMORY_E);
    sp = stack;
    ...
    int useRoot)
{
    ...
    /* Public key, root node, is top of data stack. */
    if (ret == 0) {
        XMEMCPY(stack, stackCache->stack, (word32)params->height * params->hash_len);
    ...
    if (ret == 0) {
        if (!useRoot) {
            /* Copy stack back. */
            XMEMCPY(stackCache->stack, stack, (word32)params->height * params->hash_len);
            stackCache->offset = (word32)((size_t)sp - (size_t)stack);
        }
    }
...
static int wc_hss_presign(LmsState* state, HssPrivKey* priv_key)
{
    ...
    for (i = params->levels - 2; i >= 0; i--) {
        ...
        /* u32str(type) || u32str(otstype) || I || T(1) */
        p = priv + (word32)(i + 1) * LMS_PRIV_LEN(params->hash_len);
