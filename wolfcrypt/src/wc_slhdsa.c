     ...
     int j, int cnt, byte* sig)
{
    ...
    if (ret == 0) {
        ...
        if (msg[idx[2]] != j) {
            ...
            ret = slhdsakey_chain_idx_x4_16(sk, (word32)j, (word32)(msg[idx[2]] - j), pk_seed,
                addr, idx, key->heap);
    ...
    if (ret == 0) {
        ...
        if (msg[idx[1]] != j) {
            /* Do 4 as we can't do less. */
            ret = slhdsakey_chain_idx_x4_16(sk, (word32)j, (word32)(msg[idx[1]] - j), pk_seed,
                addr, idx, key->heap);
            /* Update number of iterations performed. */
            j = msg[idx[1]];
        }
    ...
    if (ret == 0) {
        ...
        if (msg[idx[0]] != j) {
            ...
            ret = slhdsakey_chain(key, sk, (byte)j, (byte)(msg[idx[0]] - j), pk_seed, adrs,
                sk);
        }
    }
    if (ret == 0) {
     ...
     int j, int cnt, byte* sig)
{
    ...
    if (ret == 0) {
        ...
        if (msg[idx[2]] != j) {
            /* Do 4 as we can't do less. */
            ret = slhdsakey_chain_idx_x4_24(sk, (word32)j, (word32)(msg[idx[2]] - j), pk_seed,
                addr, idx, key->heap);
            /* Update number of iterations performed. */
            j = msg[idx[2]];
        }
    ...
    if (ret == 0) {
        ...
        if (msg[idx[1]] != j) {
            /* Do 4 as we can't do less. */
            ret = slhdsakey_chain_idx_x4_24(sk, (word32)j, (word32)(msg[idx[1]] - j), pk_seed,
                addr, idx, key->heap);
            /* Update number of iterations performed. */
            j = msg[idx[1]];
        }
     ...
     int j, int cnt, byte* sig)
{
    ...
    if (ret == 0) {
        ...
        if (msg[idx[2]] != j) {
            /* Do 4 as we can't do less. */
            ret = slhdsakey_chain_idx_x4_32(sk, (word32)j, (word32)(msg[idx[2]] - j), pk_seed,
                addr, idx, key->heap);
            /* Update number of iterations performed. */
            j = msg[idx[2]];
        }
    ...
    if (ret == 0) {
        ...
        if (msg[idx[1]] != j) {
            /* Do 4 as we can't do less. */
            ret = slhdsakey_chain_idx_x4_32(sk, (word32)j, (word32)(msg[idx[1]] - j), pk_seed,
                addr, idx, key->heap);
            /* Update number of iterations performed. */
            j = msg[idx[1]];
        }
    ...
    if (ret == 0) {
        ...
        if ((msg[idx[1]] != j) && (msg[idx[1]] != msg[idx[2]])) {
            ret = slhdsakey_chain_idx_x4_16(node, msg[idx[1]],
                (word32)(msg[idx[2]] - msg[idx[1]]), pk_seed, addr, idx, key->heap);
        }
    }
    if (ret == 0) {
    ...
    int z, const byte* pk_seed, word32* adrs, byte* node)
{
    ...
    if (z == 0) {
        ...
        byte n = key->params->n;
        const word32 wi = (word32)i;
        const word32 wz = (word32)z;
        ...
        if (ret == 0) {
            ...
            for (j = 0; j < m; j++) {
                ...
                HA_SetTypeAndClearNotKPA(adrs, HA_WOTS_HASH);
                /* Step 3: Set key pair address. */
                HA_SetKeyPairAddress(adrs, m * wi + j);
                /* Step 4: Generate WOTS+ public key. */
                ...
                for (k = wz - 1U; k > 0; k--) {
                    if (((j >> (wz - 1U - k)) & 1U) == 1U) {
                        ...
                        HA_SetTreeHeight(adrs, wz - k);
                        HA_SetTreeIndex(adrs, (m * wi + j) >> (wz - k));
                        /* Step 11: Calculate node from two below. */
                        ret = HASH_H(key, pk_seed, adrs, nodes + k * n,
                            n, nodes + (k - 1U + ((j >> (wz - k)) & 1U)) * n);
                        if (ret != 0) {
                            break;
                        }
                    }
...
#endif
        {
            ...
            if (ret == 0) {
                ret = wc_Shake256_Final(&key->hash.shk.shake, md,
                    (word32)key->params->dl1 + key->params->dl2 + key->params->dl3);
            }
        }
        if (ret == 0) {
    ...
    if (ret == 0) {
        ...
        if (SLHDSA_IS_SHA2(key->params->param)) {
            ...
            ret = slhdsakey_h_msg_sha2(key, sig, hdr, ctx, ctxSz, msg, msgSz,
                md, (word32)key->params->dl1 + key->params->dl2 + key->params->dl3);
        }
        else
#endif
        {
        ...
        if (ret == 0) {
            ...
            *inOutIdx += (word32)privSz;
            ...
            if (ret != 0) {
                ...
                key->flags = (byte)(oldFlags & ~WC_SLHDSA_FLAG_BOTH_KEYS);
                *inOutIdx = savedIdx;
            }
        }
        else {
    ...
    SlhDsaKey* key, word32 inSz)
{
    ...
    if (key->params != NULL && savedIdx < inSz) {
        ...
        if (ret != WC_NO_ERR_TRACE(BAD_LENGTH_E)) {
            ...
            key->flags = (byte)(oldFlags & ~WC_SLHDSA_FLAG_PUBLIC);
        }
        else {
            key->flags = oldFlags;
        }
