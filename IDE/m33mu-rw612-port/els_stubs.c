...
/* Every CLNS symbol els_pkc_port.c references, defined to fail. These simulate
 * nothing: they let this firmware link the port and assert that wolfCrypt still
 * returns correct answers in software with it registered. Each keeps the flow-
 * protection bookkeeping, so the port takes its status-driven error path. */
    ...
    mcuxClSession_Handle_t pSession)
{
    ...
    MCUX_CSSL_FP_FUNCTION_EXIT(mcuxClSession_cleanup,
...
/* This one succeeds, and that is the point: wc_ElsPkc_Init() returns early if
 * bring-up fails, which would leave nothing registered and no port for the
 * fallback test to fall back from. */
