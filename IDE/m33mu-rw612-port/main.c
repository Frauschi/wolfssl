...
/* A hardware FAILURE is not a decline: the port reports the error rather than
 * quietly finishing in software. With every CLNS call failing, an operation on
 * the port's devId must error, while the same one on the default devId must
