...
Bring the port up once at start-up, after the SDK has initialised the ELS, then pass `WOLFSSL_ELS_PKC_DEVID` as the `devId` on the wolfCrypt objects that should be offloaded:

```c
#include <wolfssl/wolfcrypt/port/nxp/els_pkc_port.h>

if (wc_ElsPkc_Init() != 0) {
    /* hardware unavailable - every algorithm still works in software */
}

wc_Sha256 sha;
wc_InitSha256_ex(&sha, NULL, WOLFSSL_ELS_PKC_DEVID);
```
...
Call `wc_ElsPkc_Cleanup()` on shutdown.
