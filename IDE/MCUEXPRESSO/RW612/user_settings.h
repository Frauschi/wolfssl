...
/* The port is a crypto callback; KEYSTORE adds the ELS key store operations.
 *
 * FIND matters more than it looks: with NO_DEV_RANDOM below there is no
 * software seed, and wc_GenerateSeed() dispatches to a device only when the
 * RNG carries a devId - which wc_InitRng() does not set. Without FIND,
 * wc_InitRng() fails with WC_HW_E and nothing falls back. */
